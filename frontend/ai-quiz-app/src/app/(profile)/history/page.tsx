...
export default function AnalyticsPage() {
    ...
    const animateNumbers = () => {
        ...
        // Convert scores to percentages for animation
        const avgScorePercent = analytics.averageScore
        const highestScorePercent = analytics.highestScore

        const increments = {
            totalQuizzes: analytics.totalQuizzes / steps,
            averageScore: avgScorePercent / steps,
            highestScore: highestScorePercent / steps
        }
        ...
        const timer = setInterval(() => {
            ...
            setAnimatedValues({
                totalQuizzes: Math.min(Math.floor(increments.totalQuizzes * currentStep), analytics.totalQuizzes),
                averageScore: Math.min(increments.averageScore * currentStep, avgScorePercent),
                highestScore: Math.min(increments.highestScore * currentStep, highestScorePercent)
            })
    ...
    // Generate dynamic colors based on category count
    ...
    // Sort category progress to show latest scores first
    const sortedCategoryProgress = analytics?.categoryProgress.map(category => ({
        ...category,
        scores: [...category.scores].reverse(), // Reverse scores array
        dates: [...category.dates].reverse()    // Reverse dates array to match
    })) || []
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-200">
                                        <th className="text-left py-3 text-gray-600 font-medium">Date</th>
                                        <th className="text-left py-3 text-gray-600 font-medium">Category</th>
                                        <th className="text-left py-3 text-gray-600 font-medium">Questions</th>
                                        <th className="text-left py-3 text-gray-600 font-medium">Score</th>
                                        <th className="text-left py-3 text-gray-600 font-medium">Actions</th>
                                    </tr>
                                </thead>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Category Progress</h3>
                    <div className="space-y-4">
                        {sortedCategoryProgress.map((category, index) => {
                            const isExpanded = expandedCategory === category.category
                            const maxScore = Math.max(...category.scores)
                            const minScore = Math.min(...category.scores)
                            const latestScore = category.scores[0] // First score after reverse
                                    ...
                                    >
                                        ...
                                        {isExpanded && category.scores.length > 0 && (
                                            <div className="mt-4 animate-in fade-in duration-300">
                                                <div className="relative h-20 mb-2">
                                                    <svg viewBox="0 0 100 40" className="w-full h-full">
                                                        {renderLineGraph(category.scores, dynamicColors[index])}
                                                    </svg>
                                                </div>
                                                <div className="flex justify-between text-xs text-gray-500">
                                                    <span>Min: {minScore}%</span>
